...
class WidgetFilepicker4ward extends Widget
{
	...
	{
		return '<a href="system/modules/filepicker4ward/files.php" onclick="Mediabox.open([[this.get(\'href\')+\'?ext='.urlencode($this->extensions).'&f=\'+encodeURI(this.getNext(\'input\').get(\'value\').replace(/\\./g,\'==PUNKT==\'))+\'&fld=\'+encodeURI(this.getNext(\'input\').get(\'name\'))+\''.(($this->filesOnly)?'&filesOnly=1':'').'\',\'\',\'800 600\']],0);return false;" title="'.$this->varValue.'">'.$this->getThumb($this->varValue).'</a>'.
